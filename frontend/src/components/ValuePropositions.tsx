      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {CAPABILITIES.map((cap) => (
          <div
            ...
            className="group relative bg-[#F7FAF9] border border-[#C4C6D0] hover:border-[#006A6A] rounded-xl p-6 flex flex-col justify-between gap-4 cursor-pointer hover:-translate-y-1 transition-all duration-200 shadow-2xs hover:shadow-md"
          >
            {/* Top row with icon & badge */}
            <div className="flex items-center justify-between">
              <div
                className={`w-12 h-12 rounded-lg ${cap.iconBg} flex items-center justify-center shadow-xs group-hover:scale-105 transition-transform`}
              >
                {getIcon(cap.iconName, cap.iconColor)}
              </div>
              <span className="text-[11px] font-mono text-[#006A6A] bg-[#EBEEED] group-hover:bg-[#8CF3F3]/40 px-2.5 py-1 rounded-md transition-colors">
                {cap.badge}
              </span>
            </div>
